	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !dryRun {
				if found == nil && createIfNotExists {
					// Create new ruleset
					ruleset, err = gh.CreateOrgRuleset(ctx, client, repository, ruleset)
					if err != nil {
						return fmt.Errorf("failed to create organization ruleset: %w", err)
					}
					logger.Info("Successfully created ruleset.", "rulesetID", *ruleset.ID, "rulesetName", ruleset.Name, "organization", repository.Owner)
				} else {
					// Update existing ruleset
					ruleset, err = gh.UpdateOrgRuleset(ctx, client, repository, *found.ID, ruleset)
					if err != nil {
						return fmt.Errorf("failed to update organization ruleset: %w", err)
					}
					logger.Info("Successfully updated ruleset.", "rulesetID", *ruleset.ID, "rulesetName", ruleset.Name, "organization", repository.Owner)
				}
			}
